...
struct FullScreenJournalView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            // Set initial prompt if not already set
    ...
    private var textEditorSection: some View {
        VStack(spacing: 0) {
            ...
            if !viewModel.attachedPhotos.isEmpty || viewModel.isUploadingPhoto {
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: 12) {
                        ForEach(Array(viewModel.attachedPhotos.enumerated()), id: \.offset) { index, photoUrl in
                            ZStack(alignment: .topTrailing) {
                                ...
                                Button(action: {
                                    Task {
                                        await viewModel.removePhoto(at: index)
                                    }
                                }) {
                                    Image(systemName: "xmark.circle.fill")
                                        .font(.system(size: 24))
    ...
    private var bottomToolbar: some View {
        HStack(spacing: 8) {
            ...
            toolbarButton(("paintpalette", "color")) {
